...
class AgentController:
    ...
    def run(self, goal: str, step_delay: float = 0.0) -> AgentState:
        ...
        return self._run_loop(state, monotonic(), step_delay=step_delay)

    def resume_after_approval(self, state: AgentState, *, approved: bool, step_delay: float = 0.0) -> AgentState:
    ...
    def _run_loop(self, state: AgentState, started: float, step_delay: float = 0.0) -> AgentState:
        import time
        ...
        while state.step_count < self._max_steps:
            ...
            try:
                raw_decision = self._decision_provider.decide(state, self._registry.discover())
