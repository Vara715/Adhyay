...
# In-memory store mapping run_id -> (AgentController, AgentState)
_RUNS: dict[str, tuple[AgentController, AgentState]] = {}
...
def _create_controller(scenario_key: str, threshold: Decimal) -> tuple[AgentController, SimulatedCompanyRepository]:
    ...
    if settings.llm_is_configured:
        try:
            ...
            class _DynamicProvider(LLMProvider):
                def complete(self, request, *, model):
                    return LLMResponse(content="Fallback", tool_call=None)

            client = LLMClient.from_settings(settings, _DynamicProvider())
...
def start_run(request: CreateRunRequest) -> AgentState:
    ...
    if request.step_delay_seconds > 0:
        ...
        _RUNS[state.run_id] = (controller, state)

        def worker():
            controller._run_loop(state, monotonic(), step_delay=request.step_delay_seconds)

        thread = threading.Thread(target=worker, daemon=True)
        thread.start()
        return state

    state = controller.run(request.goal, step_delay=0.0)
...
def submit_approval(run_id: str, decision: ApprovalDecisionRequest) -> AgentState:
    ...
    if decision.step_delay_seconds > 0:
        ...
        def worker():
            controller.resume_after_approval(state, approved=decision.approved, step_delay=decision.step_delay_seconds)

        thread = threading.Thread(target=worker, daemon=True)
        thread.start()
        return state

    updated_state = controller.resume_after_approval(state, approved=decision.approved, step_delay=0.0)
    _RUNS[run_id] = (controller, updated_state)
